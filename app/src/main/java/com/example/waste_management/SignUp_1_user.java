...
public class SignUp_1_user extends AppCompatActivity {
    boolean pass=false;
        ...
        mAuth.fetchSignInMethodsForEmail(user_email).addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                ...
                if (task.getResult().getSignInMethods().isEmpty()) {
                    ...
                    mAuth.createUserWithEmailAndPassword(user_email, user_password).addOnCompleteListener(SignUp_1_user.this, new OnCompleteListener<AuthResult>() {
                        @Override
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            if (task.isSuccessful()) {
                                ...
                                Intent intent = new Intent(SignUp_1_user.this, sign_up2_user.class);
                                startActivity(intent);
                                finish();
                            } else {
                                email.setError("Invalid email");
                                Toast.makeText(SignUp_1_user.this, "Authentication failed: " + task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
                } else {
                    ...
                    Toast.makeText(SignUp_1_user.this, "Email already registered", Toast.LENGTH_SHORT).show();
                }
            } else {
    ...
    private void registerUser1() {
        ...
        myRef.child(user_username).setValue(user).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if(task.isSuccessful()){
                    Toast.makeText(SignUp_1_user.this, "User saved successfully", Toast.LENGTH_SHORT).show();
                }
                else{
                    Toast.makeText(SignUp_1_user.this, "Failed to save user: " + task.getException().getMessage(), Toast.LENGTH_LONG).show();
                }
            }
        });
    }
